#!/usr/bin/env bash
# Corre los nueve oráculos. Sale distinto de 0 si alguno falla.
# Uso: BASE=http://localhost:3000 scripts/oraculos/correr.sh
set -uo pipefail
cd "$(dirname "$0")/../.."
fallos=0
for o in contraste barra-viva paridad-i18n render enlaces anclas barra-tapa alcanzabilidad movil; do
  echo ""
  echo "──────── $o ────────"
