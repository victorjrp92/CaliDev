...
export function PayrollPreview() {
  return (
    <PreviewShell title="Liquidación · semana 31">
      <div className="flex items-center justify-between rounded-lg bg-[#E6E8E3] px-3 py-2">
      ...
      <ul className="mt-2.5 flex flex-col gap-1.5">
        {ROWS.map((row) => (
          ...
          >
            <span className="rounded bg-[#F5F2EC] px-1.5 py-0.5 font-mono text-[8px] text-[#77847C]">
              {row.code}
            </span>
            <span className="flex-1 truncate text-[11px] font-semibold">
              {row.name}
            </span>
            <span className="font-mono text-[9px] text-[#77847C]">
              {row.hours}
            </span>
            <span className="font-mono text-[10px] font-bold text-[#14201B]">
              {row.pay}
            </span>
          </li>
        ))}
      ...
      <div className="mt-2.5 flex gap-1.5">
        {["Salud", "Pensión", "ARL", "Caja"].map((item) => (
          <span
            key={item}
            className="flex-1 rounded-md bg-[#F5F2EC] py-1 text-center text-[8.5px] font-semibold text-[#46554D]"
          >
            {item} ✓
          </span>
        ))}
